...
type Props = {
  ...
  preSummary: string;
  preAnxiety: number;
  onComplete: (summary: string, postAnxiety: number) => void;
  onClose: () => void;
};

export default function ActionReflectChat({ missionTitle, preSummary, preAnxiety, onComplete, onClose }: Props) {
  ...
  // 冒頭の問いかけ（表示専用。APIには送らない＝予測はsystemに入っている）
  const opening = `おかえりなさい。挑む前は「${preSummary}」と話していましたね。\n実際は、どうでしたか？　起きたこと・感じたことを聞かせてください。`;
  ...
  async function sendMessage() {
    ...
    try {
      const res = await fetch('/api/action-prep', {
        ...
        body: JSON.stringify({ action: 'reflect', missionTitle, preSummary, preAnxiety, messages: newMessages }),
      });
  ...
  async function handleSummarize() {
    ...
    try {
      const res = await fetch('/api/action-prep', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'reflect-summarize', preSummary, messages }),
      });
      const data = await res.json();
    ...
    return (
      <div className="fixed inset-0 z-50 flex flex-col bg-white">
        <div className="flex items-center gap-3 px-4 py-3 border-b border-stone-100">
          ...
          <p className="text-sm font-bold text-stone-700">今日のふりかえり</p>
        </div>
        <div className="flex-1 overflow-y-auto px-5 py-6 space-y-6">
          <div className="bg-stone-50 rounded-2xl p-4 border border-stone-100 space-y-1">
            <p className="text-xs text-stone-400">挑む前の予測</p>
            <p className="text-sm text-stone-600 leading-relaxed">{preSummary}</p>
          </div>
          ...
          <div className="space-y-3">
            <p className="text-sm font-bold text-stone-700">いま、この不安はどれくらいですか？</p>
            <div className="flex gap-2">
              {[1,2,3,4,5].map(n => (
                <button key={n} onClick={() => setPostAnxiety(n)} className={`flex-1 h-10 rounded-full text-sm font-bold transition-colors ${postAnxiety >= n ? 'bg-teal-400 text-white' : 'bg-stone-100 text-stone-400'}`}>●</button>
              ))}
            </div>
            <div className="flex justify-between text-xs text-stone-300"><span>小さい</span><span>大きい</span></div>
            <p className="text-xs text-stone-400 text-center">挑む前は {preAnxiety} / 5 でした</p>
          </div>
        </div>
        <div className="px-4 py-3 border-t border-stone-100">
          <button
            onClick={() => onComplete(summary, postAnxiety)}
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-white">
      <div className="flex items-center gap-3 px-4 py-3 border-b border-stone-100">
        ...
        <p className="text-sm font-bold text-stone-700 truncate">挑んだ後のふりかえり</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-4 pb-4">
        <div className="space-y-3">
          ...
          <div className="flex justify-start">
            <div className="max-w-[80%] px-4 py-3 rounded-2xl rounded-bl-md text-sm leading-relaxed whitespace-pre-wrap bg-white border border-stone-100 text-stone-700">
              {opening}
            </div>
          </div>
          {messages.map((msg, i) => (
      ...
      <div className="flex gap-2 items-end px-4 py-3 border-t border-stone-100">
        <textarea
          ...
          placeholder="実際どうだったか、書いてみてください..."
          rows={2}
          disabled={loading}
