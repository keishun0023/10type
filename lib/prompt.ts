...
注意：
- welcomeSteps は4〜5ステップ。流れ：①ようこそ ②あなたは今こういう状態でした ③消耗の正体 ④だから何をやるか ⑤一緒に進もう、の順。userInsight/レポートと一貫させる。具体的な場面名はコピーしない。
...
この人の診断結果をもとに、ペイウォールで見せる2つの要素を作ってください。恐れは性格ではなく、脳が身を守るために作った回路・パターンとして描きます。

${orientationGuide}

(A) 分析結果の提示（画面1で表示）
- fears: この人の心の奥にある恐れを提示する。「あなたの心の奥には、『〜』と『〜』への恐れがあるようです。」の形を基本に、上位の恐れ2つを本人の言葉のニュアンスで具体的に言い表す。1〜2文。
- consequence: その恐れの結果、どんな行動・反応パターンが起きやすくなっているかを描く。「その結果、〜してしまう——そんなパターンが起きやすくなっています。」の形。1〜2文。
- dailyStruggles: その恐れが根っこにありそうな、日常での具体的な悩みを3つ。本人の場面・自由記述を踏まえつつ一段具体化した短い一文（各20〜35文字）。箇条書きで見せる素材。

(B) 30日後の変化（画面3で表示）
- outcomes: このプログラムを30日続けた先に期待できる変化を4つ。各20〜35文字の前向きな一文。「〜が増える」「〜できるようになる」「〜がやわらぐ」のように頻度・可能性で表現する。

以下のJSON形式で**JSONのみ**を出力してください：

{
  "fears": "心の奥にある恐れの提示。1〜2文。",
  "consequence": "その結果起きやすいパターン。1〜2文。",
  "dailyStruggles": ["日常の悩み1", "日常の悩み2", "日常の悩み3"],
  "outcomes": ["30日後の変化1", "30日後の変化2", "30日後の変化3", "30日後の変化4"]
}

注意：
- 本人の書いた具体的な場面名をそのままコピーせず、一段抽象化して扱う。
- dailyStruggles と outcomes は指定の件数を必ず守る。
- 「治る」「必ず」「絶対」など確実性・医療表現は使わない。`;
